use rust_aoc2020::days::{
    day01, day02, day03, day04, day05, day06, day08, day09, day10, day11, day12, day13, day14,
    day15, day16, day17, day18, day19, day20,
};

fn main() {
    /*
    let input_1 = day01::load_input("input");
    println!("Day 1 {}", day01::one(&input_1));
    println!("Day 1 {}\n", day01::two(&input_1));

    let input_2 = day02::load_input("input");
    println!("Day 2 {}", day02::one(&input_2));
    println!("Day 2 {}\n", day02::two(&input_2));


    let input_3 = day03::load_input("input");
    println!("Day 3 {}", day03::one(&input_3));
    println!("Day 3 {}\n", day03::two(&input_3));


    let input_4 = day04::load_input("input");
    println!("Day 4 {}", day04::one(&input_4));
    println!("Day 4 {}\n", day04::two(&input_4));

    let input_5 = day05::load_input("input");
    println!("Day 5 {}\n", day05::both(&input_5));

    let input_6 = day06::load_input("input");
    println!("Day 6 {}", day06::one(&input_6));
    println!("Day 6 {}\n", day06::two(&input_6));
     */

    /*
    // Day 7 ragequit
    let input_7 = day07::load_input("input");
    println!("Day 6 {}", day07::one(&input_7));
    println!("Day 6 {}\n", day07::two(&input_7));
     */

    /*
    let input_8 = day08::load_input("input");
    println!("Day 8 {}", day08::one(&input_8));
    println!("Day 8 {}\n", day08::two(&input_8));

    let input_9 = day09::load_input("input");
    println!("Day 9 {}", day09::one(&input_9));
    println!("Day 9 {}\n", day09::two(&input_9, None));


    let input_10 = day10::load_input("input");
    println!("Day 10 {}", day10::one(&input_10));
    println!("Day 10 {}\n", day10::two(&input_10));

    let input_11 = day11::load_input("input");
    println!("Day 11 {}", day11::one(&input_11));
    println!("Day 11 {}\n", day11::two(&input_11));

    let input_12 = day12::load_input("input");
    println!("Day 12 {}", day12::one(&input_12));
    println!("Day 12 {}\n", day12::two(&input_12));

    let input_13 = day13::load_input("input");
    println!("Day 13 {}", day13::one(&input_13));
    println!("Day 13 {}\n", day13::two(&input_13));

    let input_14 = day14::load_input("input");
    println!("Day 14 {}", day14::one(&input_14));
    println!("Day 14 {}\n", day14::two(&input_14));

    let input_15 = day15::load_input("input");
    println!("Day 15 {}", day15::one(&input_15));
    println!("Day 15 {}\n", day15::two(&input_15));

    let input_16 = day16::load_input("input");
    println!("Day 16 {}", day16::one(&input_16));
    println!("Day 16 {}\n", day16::two(&input_16));

    let input_17 = day17::load_input("input");
    println!("Day 17 {}", day17::one(&input_17));
    println!("Day 17 {}\n", day17::two(&input_17));

    let input_18 = day18::load_input("input");
    println!("Day 18 {}", day18::one(&input_18));
    println!("Day 18 {}\n", day18::two(&input_18));

    let input_19_1 = day19::load_input("input_1");
    println!("Day 19 {}", day19::one(&input_19_1));
    let input_19_2 = day19::load_input("input_2");
    println!("Day 19 {}\n", day19::two(&input_19_2));


     */


    let input_20 = day20::load_input("input");
    println!("Day 20 {}", day20::one(&input_20));
    println!("Day 20 {}\n", day20::two(&input_20));
}
