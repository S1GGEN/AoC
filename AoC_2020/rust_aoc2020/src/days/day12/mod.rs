...
fn calculate_two(input : &str) -> Option<i32> {
    ...
    for line in input.lines() {
        let (operator, value) = line.split_at(1);
         ...
         match operator {
            "L" => {
                let (new_waypoint_lat, new_waypoint_long) = rotate_right(waypoint_rel_latitude, waypoint_rel_longitude, value, -1).unwrap();
            ...
            &_ => {
                let new_facing : Facing = match_facing_on_str(operator).unwrap();
                let (lat, long) = get_new_pos(waypoint_rel_latitude, waypoint_rel_longitude, new_facing, value);
...
fn rotate_right(waypoint_lat : i32, waypoint_long : i32, value : &str, modifier : i32) -> Option<(i32, i32)> {
    match value {
